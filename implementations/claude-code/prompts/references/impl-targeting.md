...
**Command:**
```bash
polywave-tools resolve-impl --impl <value> --repo-dir <path>
```
...
**1. Slug:**
```
/polywave wave --impl tool-journaling
```
- Resolve via `polywave-tools list-impls` to find matching `slug` field
- Most user-friendly (short, memorable)

**2. Filename:**
```
/polywave wave --impl IMPL-tool-journaling.yaml
```
...
```
/polywave wave --impl docs/IMPL/IMPL-tool-journaling.yaml
/polywave wave --impl /abs/path/to/docs/IMPL/IMPL-tool-journaling.yaml
```
...
**Example:**
```
/polywave wave --impl tool-journaling --auto
```
...
**No pending IMPLs:**
- Report: "No pending IMPL docs found. Use `/polywave scout <feature>` to create one."
...
[
  {
    ...
    "resume_command": "/polywave wave --impl tool-journaling"
  }
]
```
...
3. If failed agents exist:
   - Use `polywave-tools build-retry-context` to get structured failure context
...
- `polywave-tools resume-detect` returns non-empty array

**Fresh start triggers:**
- No worktrees for current wave
- IMPL doc shows next wave is unchecked
- `polywave-tools resume-detect` returns empty array

## Cross-Repo IMPL Docs

**Problem:** IMPL doc lives in repo A, but orchestrates changes in repo B.

**Solution:** Always use **absolute paths** when passing manifest path to polywave-tools commands:
```bash
polywave-tools finalize-wave "$(realpath docs/IMPL/IMPL-feature.yaml)" --wave 1 --repo-dir /path/to/repo-b
```
...
```bash
polywave-tools build-retry-context --impl-doc "<path>" --wave <N> --agent <ID>
```

## Integration with Orchestrator Flow

1. User invokes `/polywave wave` or `/polywave status`
2. Call `polywave-tools resolve-impl --impl <value>` (or omit --impl for auto-select)
3. Parse JSON output to get `impl_path` and `slug`
4. Run `polywave-tools resume-detect` → check for interrupted session
