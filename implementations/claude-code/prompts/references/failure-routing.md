...
```bash
polywave-tools build-retry-context "<manifest-path>" --agent "<ID>"
```
...
If any agent reports `status: blocked` due to an interface contract being unimplementable as specified, the wave does not merge. Mark the wave BLOCKED, revise the affected contracts in the IMPL doc, and re-issue prompts to all agents whose work depends on the changed contract. Use `polywave-tools update-agent-prompt "<manifest-path>" --agent "<id>" < new-prompt.txt` to update affected agent prompts and `polywave-tools check-conflicts "<manifest-path>"` to verify no ownership conflicts before re-launching. Agents that completed cleanly against unaffected contracts do not re-run. The wave restarts from WAVE_PENDING with the corrected contracts.
...
```bash
polywave-tools scan-stubs <file1> <file2> ...
```
...
In the web app, this runs automatically after `finalize-wave`. CLI users can also run `polywave-tools validate-integration` manually and review the integration report before proceeding to the next wave.
