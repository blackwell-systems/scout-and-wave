...
**Trigger:** After `polywave-tools finalize-wave` succeeds.
...
4. Run the command shown in stderr (typically: `polywave-tools run-integration-agent "<manifest-path>" --wave <N>`)
...
Integration agent model is read from `polywave.config.json` under `agent.integration_model`. If empty, inherits parent session's model.

## E27 Planned Integration Waves

If the IMPL doc includes a wave with `type: integration`, use `polywave-tools run-integration-wave` instead of reactive gap detection. Planned integration waves run before gap detection (between steps 3 and 8a).
