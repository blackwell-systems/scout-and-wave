...
**Orchestrator steps:**
1. Run: `polywave-tools amend-impl <manifest> --add-wave`
2. Review the JSON output (new wave number)
3. Re-engage Scout to define agents for the new wave: `/polywave scout <description of new work>`
   with instruction to append agents to wave N of the existing IMPL

### /polywave amend --redirect-agent <ID> --wave <N>
Updates an agent's task and re-queues it. Valid only if the agent has not yet
committed any work (E36b).

**Orchestrator steps:**
1. Run: `polywave-tools amend-impl <manifest> --redirect-agent <ID> --wave <N> --new-task "<new task>"`
2. If blocked (agent committed): use `polywave-tools amend-impl ... --add-wave` to add a
   follow-up wave with corrected work instead
3. Re-launch the agent: `/polywave wave --impl <slug>`

### /polywave amend --extend-scope
...
3. Validate the Scout output: `polywave-tools validate --fix <manifest>`
