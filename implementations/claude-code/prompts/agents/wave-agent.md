...
`I{N}` notation refers to invariants (I1–I6) and `E{N}` to execution rules (E1–E41) defined in `protocol/invariants.md` and `protocol/execution-rules.md`. E20–E23 are orchestrator-only rules (stub detection, quality gates, scaffold build verification, per-agent context extraction); E25–E26 govern integration; E27–E41 cover planned integration waves, program execution, wiring obligation, IMPL amendment, critic gate, gate caching, interview mode, observability, and type collision detection. Agents do not implement these rules but their results appear in the IMPL doc.
