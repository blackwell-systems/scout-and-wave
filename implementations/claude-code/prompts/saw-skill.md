...
For **markdown IMPL docs** (`.md`):
```
<!-- IMPL doc: /abs/path/to/IMPL-feature.md | Wave N | Agent X -->
Read your full agent brief from the IMPL doc above. Find section "### Agent X —" and follow it exactly. Your worktree branch wave{N}-agent-{X} is already checked out. Begin immediately.
```
