...
6. **E15: IMPL doc completion marker.** If this was the final wave and post-merge verification passed, write `**Status:** COMPLETE` and `**Completed:** {ISO date}` to the IMPL doc header (after the title, before the Feature line), then commit the IMPL doc update. This is the formal close of the IMPL lifecycle. Do not write the marker if more waves remain.
