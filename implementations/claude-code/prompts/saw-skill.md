...
6. **E15: IMPL doc completion marker.** If this was the final wave and post-merge verification passed, write `<!-- SAW:COMPLETE YYYY-MM-DD -->` (with today's date) on the line immediately after the IMPL doc title (`# IMPL: ...`), then commit the IMPL doc update. This is the formal close of the IMPL lifecycle. Do not write the marker if more waves remain.
