...
4. **E16 + M4: Finalize IMPL doc before review.** After Scout writes the IMPL doc, run the atomic finalization command:
   ```bash
   sawtools finalize-impl "<absolute-path-to-impl-doc>"
   ```
   This batching command combines: (1) validate (E16 structure check), (2) populate-verification-gates (M4 gate generation), (3) validate (confirm gates valid). Exit code 0 means success; exit code 1 means failure. On failure, parse the JSON output to determine failure type:
   - **Initial validation failure:** Send errors to Scout as correction prompt: "Your IMPL doc failed validation. Fix only these sections:\n{errors}". Retry up to 3 attempts. On retry limit exhaustion, enter BLOCKED and surface errors to human.
   - **Gate population failure:** Log error but continue — verification gates can be manually written during human review.
   - **Final validation failure:** Surface to human during review (M4 bug or edge case).
