...
- `saw create-worktrees` — worktree setup for a wave
- `saw verify-commits` — commit verification before merge
- `saw scan-stubs` — E20 stub detection
- `saw merge-agents` — merge wave worktrees to main
- `saw verify-build` — post-merge build verification
- `saw cleanup` — worktree cleanup after merge
- `saw update-status` — update agent/wave status
- `saw update-context` — E18 project memory update
- `saw list-impls` — IMPL doc discovery
- `saw run-wave` — fully automated wave execution
...
For markdown IMPL docs (`.md`), use the core CLI commands where available and fall back to bash-based validation/parsing for YAML-specific operations. Scout does not yet generate YAML manifests; YAML mode is present for forward compatibility after Wave 5 Scout update.

All dual-mode detection happens transparently in the Orchestrator and CLI commands. Agents receive the same 9-field prompt structure regardless of source format. Markdown mode will be deprecated after Wave 5 completes and Scout begins generating YAML manifests.

## Execution Logic

**IMPL discovery:** Use `saw list-impls --dir "<repo-path>/docs/IMPL"` to discover existing IMPL docs in the repository. This returns a JSON array of IMPL doc metadata (path, slug, title, status). Use this for status reporting and IMPL doc selection.
...
6. **E15: IMPL doc completion marker.** If this was the final wave and post-merge verification passed:
   ...
   ```bash
   saw update-context "<manifest-path>" --project-root "<repo-path>"
   ```
