# Polywave-Teams: Agent Teams Execution Layer

Alternate execution layer for the Polywave protocol using Claude Code Agent Teams
...
Polywave today uses the raw Agent tool with `run_in_background` and manual git
...
| Invariant | Polywave (current) | Polywave-Teams |
|-----------|--------------|-----------|
...
| Field | Current (Agent tool) | Polywave-Teams (teammate) |
|-------|---------------------|---------------------|
...
Polywave-Teams option A: teammates message the lead with structured completion
data. The lead writes all reports to the IMPL doc.

Polywave-Teams option B: teammates still write to the IMPL doc directly (they
...
Agent Teams uses task dependencies (task B is blocked by task A). Polywave uses
...
## What's New (Polywave-Teams only)
...
Standard Polywave discovers protocol violations (missing completion reports,
...
`TaskCompleted` when a task is being closed. Polywave-Teams uses both to enforce
...
Polywave. Hook scripts live in `polywave-teams/hooks/` and documentation in `hooks.md`.
...
Agent Teams is experimental with limitations that affect Polywave-Teams:

1. **No session resumption:** if the lead crashes mid-wave, there's no way
   to resume teammates. Polywave's current approach (worktree branches + IMPL
   doc state) survives crashes. Polywave-Teams would lose in-progress teammates.
...
3. **No nested teams:** teammates can't spawn sub-agents. This is fine for
   Polywave (wave agents don't spawn children), but limits future extensions.

4. **Permissions set at spawn:** all teammates inherit the lead's
   permissions. Polywave agents currently get the same permissions anyway, so
   this is fine.
...
2. **Phase 1:** Prototype `polywave-teams-skill.md` with the current Agent Teams
   API. Test on a real Polywave-suitable feature. Measure: does inter-agent
