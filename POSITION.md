...
- Because the skill conforms to the open standard, it is not structurally locked to Claude Code. Any agent runtime that supports the skills standard can load the same `polywave-skill.md` file
...
- **Validation:** `validate` (E16 with --fix auto-correction), `run-critic` (E37), `set-critic-review` (critic agent result writer), `check-type-collisions` (E41), `detect-shared-types` (E45), `validate-program`, `validate-integration --wiring` (E35), `check-impl-conflicts` (P1+), `check-program-conflicts` (tier-scoped P1+ conflict check), `validate-scaffolds`, `validate-scaffold`, `freeze-check`, `pre-wave-gate` (composite readiness check: E16 + E37 + scaffold status + IMPL state), `pre-wave-validate` (E16 + E35 same-package caller gap detection), `check-callers` (whole-repo call site scanner including test files; used by Scout step 4 and Critic Check 8 to replace manual grep), `list-error-ranges` (parses `pkg/result/codes.go` and returns all allocated range prefixes; Scout uses this before defining new error constants to avoid collisions), `validate-briefs` (language-agnostic brief validation: symbol existence, line reference validity, and `wave_reference_invalid` — "Wave N Agent X" prose references validated against the actual wave assignment; Scout runs in step 17; also enforced at SubagentStop via `validate_agent_completion` hook), `check-test-cascade` (E46 pre-flight gate: detects test files referencing changed symbols that are not assigned to any agent; integrated as Step 3 in `pre-wave-validate`), `suggest-wave-structure` (validates wave ordering: callers of changed symbols in earlier waves → predicted E21A failure before worktrees are created)
- **Execution:** `run-wave`, `create-worktrees`, `create-program-worktrees`, `prepare-agent`, `journal-init`, `journal-context` (E23A), `extract-context` (E23: assembles minimal 6-section per-agent context payload — agent prompt, interface contracts, file ownership, scaffolds, quality gates, IMPL path — so agents receive only what they need; O(N) context per agent, not O(N²)), `install-hooks`, `verify-hook-installed`, `verify-isolation` (E43: confirms agent is on assigned worktree branch), `update-agent-prompt` (surgically revises one agent's task description in the IMPL doc without altering wave structure, ownership, or contracts — E8 same-wave interface failure recovery path), `debug-journal` (inspect tool execution timeline for a failed agent; `--export <file>` produces a self-contained HTML timeline for post-mortem analysis)
...
1. **Claude Code hooks** (Layer 1) -- 26 hooks across PreToolUse/PostToolUse/SubagentStart/SubagentStop/UserPromptSubmit/Stop: enforcement hooks block protocol violations (ownership I1, role separation I6, worktree isolation E43, branch drift, IMPL validation E16, pre-launch gate H5, git ownership, agent completion E42, agent isolation at startup); injection hooks prepend conditional reference content at two layers (`inject_skill_context` targets the Orchestrator via `UserPromptSubmit` + `additionalContext`, `validate_agent_launch` targets subagents via `PreToolUse` + `updatedInput` for 3 conditional references); observability hooks emit structured events for monitoring and cost tracking (`emit_agent_completion` after every agent, `auto_commit_on_write` for durability); resilience hooks guard against model failure modes (`block_claire_paths`). Stop hooks warn when sessions end with active orchestration in progress.
...
The hotfix only fires when `AllAreCascades=true` -- when even one failing file belongs to the current wave (indicating a genuine agent error), the failure escalates normally. The guard prevents the hotfix path from masking real implementation failures.

### E37: Pre-Wave Brief Review
...
**Pre-merge closed-loop gate retry (`pkg/engine/closed_loop_gate.go`).** When an individual agent's quality gate (build, test, lint) fails during `run-gates` while the agent is still in its worktree, `ClosedLoopRetry` launches a fix agent in the same worktree and re-runs the gate. Loops up to `MaxRetries` before escalating. This fires *before* merge -- the repair happens inside the existing worktree while the full agent context is still present. This is the most impactful recovery path because it catches agent mistakes before the broken result reaches the main branch.

**E7a: Environmental failure auto-correction in `--auto` mode.** Corrects three categories of environmental failure inline during wave execution without surfacing to the user: isolation failures (re-launch with explicit repo context), missing dependencies (install and re-launch), transient build errors (re-run after delay). Non-correctable failures (logic errors, test failures, interface violations) always escalate. Retry limit is 2 before escalation. Distinct from E19: E7a corrects *environmental* failures inline; E19 classifies *agent* failures after the agent reports.
...
**E28A: Stale brief refresh for mixed-tier programs.** When a bottom-up assembled PROGRAM's Tier 2 IMPLs contain briefs that reference symbols modified by Tier 1, Scout re-runs in brief-refresh mode before Tier 2 launches: ownership and wave structure are preserved, but task descriptions are updated to reference the post-Tier-1 signatures. This prevents Tier 2 agents from implementing against pre-Tier-1 API shapes when the tier boundary represents a breaking interface change.
...
**Execution:** Wave agents in git worktrees with E43 hook-based isolation enforcement (4 hooks: environment injection, auto-cd, path validation, compliance verification), solo wave optimization for single-agent waves (executes directly on branch without worktree overhead), 5-layer ownership enforcement (hooks, git pre-commit, SDK middleware, Critic Check 9 pre-wave brief review, PrepareWave fast-fail before worktree creation), E23 per-agent context extraction (O(1) context per agent regardless of wave size — each agent receives a minimal 6-section payload scoped to its own work, not the full IMPL doc containing all other agents' prompts), tool journal tracking (E23A) with checkpoint system and prior-work context injection for retries, incremental commits with auto-commit for API/Bedrock agents (async `PostToolUse` hook fires after every write, commits immediately for rate-limit resilience), file-read deduplication middleware (`pkg/agent/dedup`: SHA256 content cache intercepts repeated reads of unchanged files; second read returns a one-liner summary rather than full content; write/edit calls invalidate the cache; token savings reported in completion reports), multi-language LSP workspace setup (4 per-language implementations — Go `go.work`, TypeScript `tsconfig.json`/`package.json` project references, Python `pyrightconfig.json`/`pyproject.toml` extraPaths, Rust; each modifies workspace config to include all wave worktree paths so cross-worktree `go to definition` works; configs are backed up before modification and restored after finalization), cross-repository orchestration with coordinated merge ordering, integration waves (E27) for wiring-only work, LLM-powered code review with dimensional scoring, agent launch prioritization (critical path depth scheduling), per-role model configuration (Scout/Wave/Critic/Scaffold/Integration/Planner each configurable).
