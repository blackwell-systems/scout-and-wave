...
**Current state:** Scout, wave agents, scaffold agents, and retry attempts all use hardcoded model and backend settings. There is no per-phase optimization. A Scout run with deep reasoning needs uses the same configuration as a wave agent doing a simple file edit. Retry attempts use the exact same agent that just failed, reducing the chance of a different outcome.

**Problem:** Different orchestration phases have different needs:
- **Scout** needs deep reasoning, long context, architectural thinking (Opus 4.6, extended thinking budget)
- **Wave agents** need fast iteration, tool use, file editing (Sonnet 4.6, standard config)
- **Scaffold** needs careful type design, cross-file consistency (Opus 4.6, type-safety emphasis)
- **Retry** needs a different perspective to avoid repeating the same mistake (different prompt suffix, potentially different model)

Without phase-specific tuning, SAW either over-provisions (wastes cost/latency on simple tasks) or under-provisions (sacrifices quality on complex planning).

**Proposed:** Add `saw.config.json` in the repository root with per-phase agent configuration:

```json
{
  "protocol_version": "0.10.4",
  "agents": {
    "scout": {
      "backend": "api",
    ...
    "scaffold": {
      ...
      "model": "claude-opus-4-6",
      "max_tokens": 12000,
      "system_prompt_suffix": "CRITICAL: Maintain type safety and interface consistency across all scaffold files."
    },
    "retry": {
      "backend": "api",
      "model": "claude-sonnet-4-6",
      "max_tokens": 8000,
      "system_prompt_suffix": "IMPORTANT: You are fixing a failed attempt. Read the completion report carefully and avoid repeating the same mistake. Consider a different approach if the original strategy proved unsound."
    },
    "verify": {
      "backend": "api",
      "model": "claude-sonnet-4-6",
...
- `pkg/config/config.go` — Parse `saw.config.json` from repo root, merge with built-in defaults
- `pkg/config/defaults.go` — Sensible defaults for each phase when no config present
- `pkg/orchestrator/orchestrator.go` — Select agent config based on current phase (scout vs wave vs scaffold vs retry)
- `pkg/agent/backend/backend.go` — Accept per-invocation config overrides (model, max_tokens, temperature, system_prompt_suffix)

**Quality gate integration:**
- Auto-detect project type from marker files (`package.json`, `Cargo.toml`, `go.mod`, `pyproject.toml`)
- Run appropriate tools for each gate type:
  - TypeCheck: `tsc --noEmit` (TypeScript), `mypy .` (Python), `cargo check` (Rust), `go build` (Go)
  - Test: `npm test`, `pytest`, `cargo test`, `go test ./...`
  - Lint: `eslint .`, `ruff check .`, `cargo clippy`, `golangci-lint run`
- Gates run after each wave agent completes, before merge
- `required: false` = warning on failure, merge proceeds; `required: true` = blocks merge

**Benefits:**
- **Cost optimization** — Scout uses expensive Opus, waves use cheaper Sonnet
- **Quality optimization** — Scout gets extended thinking budget, waves don't
- **Retry diversity** — Failed attempts get modified prompts to avoid repeating mistakes
- **User control** — Power users can tune without modifying code
- **Explicit quality** — Gates surface issues before human review

**Fallback behavior:** If `saw.config.json` is absent, use built-in defaults (current behavior: API backend, claude-sonnet-4-6 for all phases). Config is opt-in, not mandatory.

**Implementation scope:** Engine only (`scout-and-wave-go`). The protocol repo documents the config schema but does not mandate it — implementations can choose not to support phase configs (e.g., a minimal shell script orchestrator). This is a quality-of-life feature for the Go implementation.

---

### Framework Skills Auto-Detection

**Current state:** Scout and wave agents receive generic system prompts with protocol rules (I1-I6, E1-E17). The prompts are framework-agnostic — a React project and a Rust project get identical guidance. If a wave agent is implementing a React component, it may violate React best practices (prop drilling, incorrect hook usage, missing keys) because those patterns were never mentioned in its context.

**Problem:** SAW users develop projects in specific frameworks with established conventions. Scout and wave agents rediscover (or violate) these conventions on every run because they lack framework-specific knowledge. The user must manually add "follow React best practices" to every feature request, or accept lower code quality.

**Proposed:** Auto-detect the project's framework from marker files and inject framework-specific skills into agent prompts:

**Detection logic:**
```
package.json + "react" dependency   → react-best-practices.md
package.json + "vue" dependency     → vue-best-practices.md
Cargo.toml exists                   → rust-ownership.md, rust-error-handling.md
go.mod exists                       → go-idioms.md, go-error-handling.md
pyproject.toml + "fastapi"          → fastapi-patterns.md
```

**Skill files stored in `scout-and-wave/skills/`:**
```
skills/
...
Each skill file is a markdown document (500-1000 words) with:
...
```go
// pkg/skills/detect.go
func DetectSkills(repoPath string) []string {
  skills := []string{}

  if hasFile(repoPath, "package.json") {
    pkg := parsePackageJSON(repoPath + "/package.json")
    if pkg.HasDependency("react") || pkg.HasDependency("next") {
      skills = append(skills, "react-best-practices")
    }
    if pkg.HasDependency("vue") || pkg.HasDependency("nuxt") {
      skills = append(skills, "vue-best-practices")
    }
  }

  if hasFile(repoPath, "Cargo.toml") {
    skills = append(skills, "rust-ownership", "rust-error-handling")
  }

  if hasFile(repoPath, "go.mod") {
    skills = append(skills, "go-idioms", "go-error-handling")
  }

  if hasFile(repoPath, "pyproject.toml") {
    cfg := parsePyprojectToml(repoPath + "/pyproject.toml")
    skills = append(skills, "python-type-hints")
    if cfg.HasDependency("fastapi") {
...
func LoadSkills(skillNames []string, skillsPath string) string {
  var content strings.Builder
  content.WriteString("\n\n## Framework-Specific Guidelines\n\n")

  for _, name := range skillNames {
    path := filepath.Join(skillsPath, name + ".md")
    data, err := os.ReadFile(path)
    if err != nil {
      continue // Skill file missing, skip silently
    }
...
**Prompt injection:** Scout and wave agents' system prompts are appended with the loaded skill content. This happens transparently — the user never sees it, but agents receive framework-specific context.

**Configuration override:** Users can disable auto-detection or specify custom skills via `saw.config.json`:
```json
{
  "framework_skills": {
    "auto_detect": true,
    "skills_path": "../scout-and-wave/skills/",
    "additional_skills": ["custom-team-patterns"]
  }
}
```

**Benefits:**
- **Zero configuration** — Auto-detected from project files
- **Improved code quality** — Agents follow framework conventions automatically
- **Consistent style** — All wave agents get the same framework guidance
- **Extensible** — Users can add custom skill files for team-specific patterns

**Implementation scope:** Core detection logic in `scout-and-wave-go` engine. Skill content lives in `scout-and-wave` protocol repo (`skills/` directory). This keeps framework knowledge in the protocol, not the implementation.

---

### Web UI Settings Panel

**Current state:** `saw serve` web UI has no configuration interface. All settings are hardcoded or controlled via CLI flags (`--addr`, `--impl-dir`, `--repo`). To change agent models, backends, or quality gates, the user must:
1. Edit `saw.config.json` manually (if they know the schema)
2. Restart the server for changes to take effect
3. Guess at valid values (no validation or autocomplete)

There is no feedback on what frameworks were detected, what skills are active, or what the current agent configuration is. The UI feels opaque — users cannot inspect or tune the orchestrator's behavior.

**Problem:** Configuration via JSON files is developer-friendly but intimidating for non-technical users. The web UI should expose settings visually, validate inputs, and provide immediate feedback. Without this, SAW remains a power-user tool rather than an accessible product.

**Proposed:** Add a Settings screen to the web UI with sections for agent configuration, quality gates, and framework skills.
...
│  │   Model:      [claude-sonnet-4-6      ▼]              ││
│  │   Backend:    [API                    ▼]              ││
│  │   Prompt:     [You are fixing a failed attempt...]    ││
│  └────────────────────────────────────────────────────────┘│
│                                                              │
│  Quality Gates                                               │
│  ┌────────────────────────────────────────────────────────┐│
│  │ ☑ Type Checking     [Auto-detect  ▼]  ☐ Required      ││
│  │ ☑ Run Tests         [Auto-detect  ▼]  ☐ Required      ││
│  │ ☑ Linting           [Auto-detect  ▼]  ☐ Required      ││
│  └────────────────────────────────────────────────────────┘│
│                                                              │
│  Framework Skills                                            │
│  ┌────────────────────────────────────────────────────────┐│
...
│  │                                                         ││
│  │ Skills directory: [../scout-and-wave/skills/]          ││
│  └────────────────────────────────────────────────────────┘│
│                                                              │
...
type ConfigResponse struct {
  ...
  QualityGates     map[string]GateConfig    `json:"quality_gates"`
  FrameworkSkills  FrameworkSkillsConfig    `json:"framework_skills"`
  Detected         DetectedContext          `json:"detected"`
}

type DetectedContext struct {
  Frameworks  []string  `json:"frameworks"`   // ["react", "typescript"]
  Skills      []string  `json:"skills"`       // ["react-best-practices.md"]
  ProjectType string    `json:"project_type"` // "node", "rust", "go", "python"
}
...
// Returns: ConfigResponse with default values

// GET /api/config/validate — Validate config before saving
...
type ValidationError struct {
  Field   string  `json:"field"`   // "agents.scout.model"
  Message string  `json:"message"` // "Invalid model name"
}
```
...
export default function SettingsScreen() {
  return (
    <div className="max-w-4xl mx-auto p-8">
      ...
      <AgentConfigSection />
      <QualityGatesSection />
      <FrameworkSkillsSection />
      <ActionButtons onSave={handleSave} onReset={handleReset} />
    </div>
  )
}

// web/src/components/Settings/AgentConfigSection.tsx
<Section title="Agent Configuration">
  {['scout', 'wave', 'scaffold', 'retry'].map(phase => (
    <AgentConfigCard key={phase} phase={phase} config={config.agents[phase]} onChange={handleChange} />
  ))}
</Section>

// web/src/components/Settings/AgentConfigCard.tsx
<Card>
  <CardHeader>{capitalize(phase)} Agent</CardHeader>
  <CardContent>
    <Select label="Model" value={config.model} options={modelOptions} onChange={...} />
    <Select label="Backend" value={config.backend} options={['api', 'cli', 'auto']} onChange={...} />
    <Input label="Max Tokens" type="number" value={config.max_tokens} onChange={...} />
    {phase === 'retry' && (
      <Textarea label="System Prompt Suffix" value={config.system_prompt_suffix} onChange={...} />
    )}
  </CardContent>
</Card>
```
...
- `claude-sonnet-4-5`
- `claude-haiku-4-5`
- Custom input for other models

**Backend dropdown options:**
- `api` — Use Anthropic API (requires `ANTHROPIC_API_KEY`)
- `cli` — Use `claude` CLI (requires Claude Max plan)
- `auto` — API if key present, CLI otherwise

**Quality gates:**
- Checkboxes for enable/disable
- Dropdown for command (`auto`, `custom`)
- If `custom`, show text input for command
- "Required" checkbox (blocks merge on failure vs warning only)

**Framework skills:**
- Checkbox for auto-detect
- Read-only display of detected frameworks and active skills
- Path input for skills directory (defaults to `../scout-and-wave/skills/`)
- Button to "Reload Skills" (re-runs detection without restarting server)

**Save behavior:**
- Validates config via `POST /api/config/validate`
- Shows validation errors inline (red text under invalid fields)
- If valid, saves via `POST /api/config`
- Writes `saw.config.json` to disk
- No restart required — config hot-reloads on save

**Benefits:**
- **Accessible** — Non-developers can tune settings without editing JSON
- **Validated** — Invalid inputs rejected before save
- **Transparent** — Users see what frameworks/skills are detected
- **Discoverable** — Settings screen reveals configuration options users didn't know existed

**Navigation:**
Add "Settings" link to top header (next to "Scout and Wave" title):
```tsx
...
**Implementation scope:** Engine only (`scout-and-wave-go`). Web UI routes, API endpoints, config persistence. No protocol changes — this is a GUI for the config schema defined in the previous item.
