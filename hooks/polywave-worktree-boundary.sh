...
#
# Environment:
#   POLYWAVE_WORKTREE_ROOT — absolute path to the agent's worktree (set by prepare-wave)
#                       When not set, this hook is a no-op (exit 0).

set -euo pipefail

# No-op when not in a worktree agent context
if [[ -z "${POLYWAVE_WORKTREE_ROOT:-}" ]]; then
  exit 0
fi
...
worktree_root=$(python3 -c "import os,sys; print(os.path.normpath(sys.argv[1]))" "$POLYWAVE_WORKTREE_ROOT" 2>/dev/null || printf '%s' "$POLYWAVE_WORKTREE_ROOT")
...
# the worktree: main_repo/.claude/worktrees/polywave/<slug>/wave<N>-agent-<ID>)
# Pattern: worktree is at <main_repo>/.claude/worktrees/polywave/*/wave*-agent-*
main_repo=""
...
if [[ -n "$main_repo" && ( "$file_path" == "$main_repo"/* || "$file_path" == "$main_repo" ) ]]; then
  echo "[Polywave] Write blocked: $file_path is in main repo, not agent worktree." >&2
  echo "[Polywave] Use: $POLYWAVE_WORKTREE_ROOT/$(python3 -c "import os,sys; print(os.path.relpath(sys.argv[1], sys.argv[2]))" "$file_path" "$main_repo" 2>/dev/null || printf '%s' "$file_path")" >&2
  exit 2
fi
