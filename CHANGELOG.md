...
- **LSP hover for signature accuracy** — `scout.md` Step 5 (interface contracts): "Before finalizing any contract referencing an existing symbol, use LSP hover to confirm the exact current signature"; `wave-agent.md` LSP rule expanded to include hover for pre-implementation signature verification (not just findReferences for call sites); `critic-agent.md` Check 2 (symbol_accuracy) now uses LSP `hover` to verify exact function signatures (param types, count, return type) for cross-agent symbol dependencies — not just grep for existence; `LSP` added to critic agent tool list
...
| [0.76.0] | 2026-03-31 | Doc updates — `POSITION.md` agent type section rewritten in current voice (post-migration architecture); `docs/skills-progressive-disclosure.md` updated with accurate hook execution order and dedup marker strings; `implementations/claude-code/hooks/README.md` updated for post-migration hook layout; `implementations/claude-code/prompts/scripts/README.md` updated for post-migration script layout; deleted stale `docs/TODO-isolation-hook-fix.md` |
| [0.75.0] | 2026-03-30 | E44 + E35 hardening — E44: automated SAW agent naming via PreToolUse hook (agent names injected from IMPL brief frontmatter, eliminating manual `saw_name` field requirement); E35: `sawtools detect-wiring` detection aid integrated into Scout prompt and E35 protocol section with test fixtures; P0 bug fixes: SAW agent naming helper + E42 branch verification; orchestrator skill updated to read brief frontmatter for agent naming |
| [0.74.0] | 2026-03-28 | Integration agent CLI workflow completion — `run-integration-agent` (E26 reactive wiring) and `run-integration-wave` (E27 planned integration waves) CLI commands added to sawtools; `finalize-wave` enhanced with human-readable gap summary (symbol, file, severity) and next-step guidance to stderr; orchestrator step 8a rewritten with 7 explicit substeps replacing meta-instruction; cross-repo IMPL (4 agents: 2 CLI commands + finalize-wave enhancement + orchestrator update) executed via parallel wave; invariants v0.26.0 audit findings applied (E43 enforcement layers, cross-repo I5 handling, I6 limitations documented); schema drift fixed (`prop-pass` integration pattern added); POSITION.md updated with E45, 60+ CLI commands catalog, SDK capabilities; pre-commit-check cross-repo validation issue documented and resolved (scout-and-wave-go@1fc6e21) |
| [0.73.0] | 2026-03-27 | Orchestrator skill simplified — `saw-skill.md` reduced from 365→183 lines (50% reduction) by extracting implementation details to 4 focused reference files; moved model selection hierarchy (86→99 lines), E16/E37/E21A validation specs (262→277), I1/I2/I5/E35/E42 protocol rules (305→342), and IMPL targeting/resume logic (209→244) to `references/model-selection.md` (81 lines), `references/pre-wave-validation.md` (151 lines), `references/wave-agent-contracts.md` (137 lines), and `references/impl-targeting.md` (195 lines); preserved entire synchronous orchestration flow (bootstrap → scout → validate → critic → scaffold → wave → finalize), all decision points, command invocations, and error handling; simplified reference injection table (44→3 lines), consolidated I6 explanations (18→9 lines); install.sh wildcard pattern automatically symlinks new reference files (17→21 files); all 15 hooks (11 original + 4 E43) verified functional and properly installed |
| [0.72.0] | 2026-03-27 | Wave agent worktree isolation docs simplified — `wave-agent-worktree-isolation.md` rewritten to focus exclusively on v0.65.0+ hook-based enforcement; removed Step 0 verification workflow (hooks handle automatically), manual `$WORKTREE` capture patterns (use `$SAW_AGENT_WORKTREE` set by `inject_worktree_env`), and "NEVER do this" anti-patterns (hooks prevent); added environment variables reference, failure escalation protocol, Bash non-persistence explanation; preserved git -C flag requirement (hooks don't modify git), go.mod replace directive warning, troubleshooting section; reduced from 115→93 lines (19% reduction) while preserving all critical information per deep review analysis |
| [0.71.0] | 2026-03-27 | E45: Shared Data Structure Scaffold Detection — Scout automatically detects types referenced by 2+ agents and emits scaffolds; I2 expanded to cover structs/enums/traits (not just function signatures); Procedure 2 updated to clarify scaffolds include detected types; `scout-implementation-process.md` integrated automated `sawtools detect-shared-types` invocation after agent prompt writing (step 10); prevents I1 violations from duplicate type definitions across agents |
| [0.70.0] | 2026-03-26 | Progressive disclosure doc audit — `docs/skills-progressive-disclosure.md` updated with accurate hook execution order (Checks 11-13 before Check 1, then Check 10), correct dedup marker strings for all 5 agent types (scout, wave-agent, critic-agent, planner, integration-agent), and clearer extension guidance for adding new agent type injection blocks |
| [0.69.0] | 2026-03-25 | Onboarding docs audit — `sawtools init` documented in CHANGELOG, scout-and-wave-go README (Setup commands table + Getting Started), init_cmd.go help text improved (mentions `/saw scout`, links to docs); web UI settings empty state mentions `sawtools init` |
| [0.68.0] | 2026-03-25 | Protocol docs conformance — all 15 protocol-reality-gap items resolved: 7 minor discrepancies fixed (version refs, flag ordering, gate level docs, merge command docs), 8 undocumented behaviors documented (active-impl write, resume detection, repo match validation, E16_REPO_MISMATCH_SUSPECTED, commit reachability, closed-loop retry, --no-prioritize, pre-wave-gate); `protocol-reality-gap.md` deleted |
| [0.67.0] | 2026-03-25 | Interview mode gap fixes (G1-G11) — `INTERVIEWING` state added to `state-machine.md` (catalog, transitions, entry actions, allowed transitions table); `interview-mode.md` Output Contract updated to actual bootstrap-oriented compiler format; `back` navigation and `--non-interactive` flag documented; parallel-systems-audit.md Item A corrected (web app DOES need cross-repo, missing 8 enforcement steps) |
| [0.66.0] | 2026-03-25 | Cross-repo file ownership fix — Scout prompt updated with "mismatched repos" warning to set `repo:` on every file_ownership entry when IMPL doc repo differs from code repo; IMPL-saw-init quality gates tagged `repo: scout-and-wave-go`; IMPL-file-browser-cross-repo archived to complete/ |
| [0.65.0] | 2026-03-24 | HOOKS.md conformance update — added full documentation for `warn_stubs` (E20) and `check_branch_drift` (I4), added both to overview table and installation section, added `emit_agent_completion` to overview table, removed stale "not yet implemented" claims, updated troubleshooting; POSITION.md rewritten with multi-provider story, three execution modes (skill/CLI/SDK), agent skills open standard conformance, provider independence section; ROADMAP.md updated with PROGRAM execution hardening items (P1-P13) and technical debt section |
| [0.64.0] | 2026-03-23 | Hook exit code fix — all 4 blocking hooks corrected `exit 1` → `exit 2`; I1 (`check_wave_ownership`), I6 (`check_scout_boundaries`), E16 (`validate_impl_on_write`), H5 (`validate_agent_launch`) enforcement now actually blocks instead of silently logging |
| [0.63.0] | 2026-03-23 | H5 pre-launch validation gate — `validate_agent_launch` CLI hook (8 checks: SAW tag, IMPL exists, IMPL valid, wave exists, agent exists, worktree branch, scaffolds committed, critic review); install.sh updated (10 hooks total) |
| [0.62.0] | 2026-03-23 | Hooks quick wins (H2/H3/H4) — `check_impl_path` (PreToolUse/Agent, blocks missing IMPL docs), `warn_stubs` (PostToolUse/Write|Edit, non-blocking stub warnings), `check_branch_drift` (PostToolUse/Bash, blocks main/master commits); install.sh updated (9 hooks); README.md rewritten with full hook documentation |
| [0.61.0] | 2026-03-23 | I1 layer 2 git ownership hook — `check_git_ownership` (PostToolUse/Bash) detects when wave agents' git commands modify files outside ownership list; catches merge conflict resolutions that bypass Write/Edit hooks; install.sh updated |
