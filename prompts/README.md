...
|------|---------|---------|
| [`saw-skill.md`](saw-skill.md) | v0.3.6 | The `/saw` skill router. Install to `~/.claude/commands/saw.md`. Routes `bootstrap`, `scout`, `wave`, `wave --auto`, and `status` commands. Drives all protocol state transitions as the Orchestrator. |
...
|------|---------|-------------|---------|
| [`scout.md`](scout.md) | v0.3.7 | Scout | Suitability gate (5 questions) + IMPL doc production. Analyzes the codebase, assigns file ownership, defines interface contracts, structures waves, and stamps per-agent prompts. May produce type scaffold files (shared interfaces, traits, structs) committed to HEAD before any worktree is created. Never modifies existing source files. |
| [`agent-template.md`](agent-template.md) | v0.3.7 | Wave Agent | 9-field prompt template stamped per-agent by the Scout into the IMPL doc. Field 0: isolation verification (mandatory pre-flight). Fields 1–8: file ownership, interfaces, implementation spec, tests, verification gate, constraints, completion report. |
